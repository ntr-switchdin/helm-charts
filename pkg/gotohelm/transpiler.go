...
func (t *Transpiler) transpileExpr(n ast.Expr) Node {
	...
	case *ast.BinaryExpr:
		...
		mapping := map[[3]string]func(a, b Node) Node{
			...
			{"float32", token.QUO.String(), "float32"}: wrapWithCast("divf", "float64"),
			{"float32", token.REM.String(), "float32"}: wrapWithCast("modf", "float64"),
			...
			{"float64", token.REM.String(), "float64"}: wrapWithCast("modf", "float64"),
